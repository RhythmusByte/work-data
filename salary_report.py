...
def is_salary_row(row):
    category = str(row[1] or "").strip().lower()
    notes = str(row[3] or "").strip().lower()
...
salary_rows = [r for r in expense_rows if is_salary_row(r)]
print(f"Salary rows matched: {len(salary_rows)}")

if not salary_rows:
    raise SystemExit("No salary rows found. Nothing to generate.")

all_dates = [parse_any_date(r[0]) for r in salary_rows]
...
# Category list for salary rows, in the order first seen (Summary sheet will re-sort by amount)
seen = []
for r in salary_rows:
    cat = (r[1] or "Uncategorised").strip()
    if cat not in seen:
        seen.append(cat)
print(f"Distinct salary-related categories found: {len(seen)}")
for c in seen:
    print("  -", c)
print("\n>>> Check this list. If two rows are really the same salary category spelled "
      "differently, fix the spelling in the source file and re-run, rather than "
      "editing the output by hand - otherwise next month's file will split them again.")
...
ws_sum = wb.active
ws_sum.title = "Salary Summary"
ws_det = wb.create_sheet("Salary Details")
...
det_last_row = ws_det.max_row  # last data row in Salary Details
...
for cat in categories_ordered:
    ...
    amt_cell = ws_sum.cell(
        row, 3,
        f"=SUMIF('Salary Details'!$B$2:$B${det_last_row},$B{row},"
        f"'Salary Details'!$C$2:$C${det_last_row})"
    )
    amt_cell.number_format = NUMFMT
    cnt_cell = ws_sum.cell(
        row, 4,
        f"=COUNTIF('Salary Details'!$B$2:$B${det_last_row},$B{row})"
    )
...
out_name = f"Salary_Summary_{period_start.strftime('%Y-%m-%d')}_to_{period_end.strftime('%Y-%m-%d')}.xlsx"
wb.save(out_name)
print("\nSaved:", out_name)
